...
class Rohith(object):
    ...
    def __init__(self, root):
        ...
        self.root.title("Pharmacy Billing System By Rohith..")
        self.root.iconbitmap(r"ui.ico")
        messagebox.showinfo("info","Welcome to Panda Pharmacy Store")
        messagebox.showinfo("info","Developed by Saravanan, Gulshan, Deepak")
...
# Coded By Rohithaditya
        ...
        F1 = LabelFrame(text="Customer Information", font=("Calibri", 12, "bold"), fg="black", bg=bg_color,
        relief=RAISED, bd=10)
        ...
        F2 = LabelFrame(self.root, text='Medicine Capsule', bd=10, relief=RAISED, bg=bg_color, fg="black",
        ...
        F2 = LabelFrame(self.root, text='Vitamins', bd=10, relief=RAISED, bg=bg_color, fg="black",
        ...
        F2 = LabelFrame(self.root, text='Cream and Ointment', bd=10, relief=RAISED, bg=bg_color, fg="black", font=("Calibre", 13, "bold"))
        ...
        F4 = LabelFrame(self.root, text='Total Menu', bd=10, relief=RAISED, bg=bg_color, fg="black",
        font=("Calibri", 13, "bold"))
        ...
        total_btn = Button(F4, text="Total", bg=bg_color, fg=fg_color, font=("lucida", 12, "bold"), bd=7, relief=RAISED,command=self.total_section)
    ...
    def welcome_customer(self):
        ...
        self.txt.insert(END, "   Welcome To Sarvans Pharmacy Store \n")
...
# This function for Add Product name , qty and price to bill section
    ...
    def billing_section(self):
        ...
        if len(self.customer_contact_number.get()) < 1:
             messagebox.showerror("Error","Phone Number must be filled")
             return

        elif len(self.customer_contact_number.get()) < 10:
             messagebox.showerror("Error","Phone Number is Not Correct")
             messagebox.showerror("Error",f"{10-len(self.customer_contact_number.get())} More Digits Needed")
             return
        
        elif len(self.customer_contact_number.get()) > 10:
             messagebox.showerror("Error","Phone Number is Not Correct")
             messagebox.showerror("Error",f"{len(self.customer_contact_number.get())-10} less Digits Needed")
             return

        elif len(self.customer_name.get()) < 1:
            messagebox.showerror("Error","Customer Name must be filled")
            return
       
        elif len(self.total_medicine.get()) < 1:
            messagebox.showerror("Error","No Items in cart/genrate total first....")
            return
        
        elif len(self.total_cream_ointment.get()) < 1:
        ...
        self.txt.insert(END, "\n--------------------------------------")
        self.txt.insert(END, f"\nMedicine Tax Value : Rs. {str(round(self.total_medicine_prices * 0.05))}")
        self.txt.insert(END, f"\nVitamins Tax Value : Rs. {str(round(self.total_vitamins_prices * 0.05))}")
        self.txt.insert(END, f"\nCream Ointment Tax Value : Rs. {str(round(self.total_cream_ointment_prices * 0.05))}")
        self.txt.insert(END, "\n--------------------------------------")
    ...
    def save_bill(self):
        ...
        if op>0:
            if not os.path.exists("Bills"):
                ...
                f1 = open("Bills/"+str(self.cus_invoice_no.get())+".txt","w")
                f1 = open("Bills/"+str(self.cus_invoice_no.get())+".txt","w")
            ...
            if os.path.exists(f"Bills/{self.cus_invoice_no.get()}"):
            ...
            f1.write(self.bill_data)
            f1.close
            messagebox.showinfo("saved", f"Bill no. : {self.cus_invoice_no.get()} saved successfully")
        else:
            return
